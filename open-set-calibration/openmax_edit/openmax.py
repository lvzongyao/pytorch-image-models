...
from openmax_edit.evt_fitting import weibull_tailfitting
from openmax_edit.compute_openmax import recalibrate_scores
...
def build_weibull(mean, distance, tail, distance_type=None):
    weibull_model = {}
    for i in range(len(mean)):
        weibull_model[label[i]] = {}
        weibull = weibull_tailfitting(mean[i], distance[i], tailsize=tail, distance_type=distance_type)
        weibull_model[label[i]] = weibull
    return weibull_model


def compute_openmax(model, imagearr, mean_path=None, distance_path=None, distance_type='euclidean'):
    if mean_path is None or distance_path is None:
        ...
        distance = np.load(distance_path, allow_pickle=True)
    ...
    for alpha in alpharank_list:
        ...
        for tail in tail_list:
            ...
            # print ('++++++++++++++++++++++++++++')
            weibull_model = build_weibull(mean, distance, tail, distance_type=distance_type)
            openmax_vector, softmax_vector, openmax_fc8= recalibrate_scores(weibull_model, label, imagearr, alpharank=alpha,
            ...
            #    total += 1
            # print ('----------------------------')
    # return np.argmax(softmax_vector), np.argmax(openmax_vector)
    return softmax_vector, openmax_vector, openmax_fc8
